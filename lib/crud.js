    ...
    try {
        ...
        if (adaptr.configCreateHtmlList) {
            await adaptr.setStateChangedAsync(`devices.${dpSubFolder}linkQualityListHTML`, {
                val: await adaptr.crud.createListHTML(adaptr,  'linkQualityList', adaptr.linkQualityDevices, adaptr.linkQualityCount, null),
                ack: true,
            });
            await adaptr.setStateChangedAsync(`devices.${dpSubFolder}offlineListHTML`, {
                val: await adaptr.crud.createListHTML(adaptr,  'offlineList', adaptr.offlineDevices, adaptr.offlineDevicesCount, null),
                ack: true,
            });
            await adaptr.setStateChangedAsync(`devices.${dpSubFolder}batteryListHTML`, {
                val: await adaptr.crud.createListHTML(adaptr,  'batteryList', adaptr.batteryPowered, adaptr.batteryPoweredCount, false),
                ack: true,
            });
            await adaptr.setStateChangedAsync(`devices.${dpSubFolder}lowBatteryListHTML`, {
                val: await adaptr.crud.createListHTML(adaptr,  'batteryList', adaptr.batteryLowPowered, adaptr.lowBatteryPoweredCount, true),
                ack: true,
            });
